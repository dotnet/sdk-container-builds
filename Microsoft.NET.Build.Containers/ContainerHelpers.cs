...
public static class ContainerHelpers
{
    ...
    public static async Task Containerize(DirectoryInfo folder, string workingDir, string registryName, string baseName, string baseTag, string[] entrypoint, string[] entrypointArgs, string imageName, string? userName, string[] imageTags, string outputRegistry, string[] labels, Port[] exposedPorts, (string key, string value)[] envVars)
    {
        ...
        {
            // env vars are validated by System.CommandLine API
        ...
        foreach (var tag in imageTags)
        {
            ...
            else
            {
                try
                {
                    ...
                    outputReg?.Push(img, imageName, tag, imageName, msg => Console.WriteLine(msg)).Wait();
