...
namespace System.Containers.Tasks
{
    public class CreateNewImage : Microsoft.Build.Utilities.Task
    {
        ...
        {
            // Either PublishDirectory or Files must have a value, otherwise error.
            if ((Files == null || Files.Length == 0) &&
                (string.IsNullOrEmpty(PublishDirectory) || !Directory.Exists(PublishDirectory)))
            {
            ...
            // Prefer directory first. Preference can be handled by an optional bool in the future
            if (Directory.Exists(PublishDirectory))
            {
                Log.LogMessage("Loading from directory");
                Layer newLayer = Layer.FromDirectory(PublishDirectory, WorkingDirectory);
                image.AddLayer(newLayer);
            }
            else if (Files != null && Files.Length != 0)
            {
                Log.LogMessage("Loading from files");
